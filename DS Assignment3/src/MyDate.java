 ...
 * value Methods: default constructor - sets date to Jan 1, 2018 initial
 * constructor (day, month, year) - sets date to parameters passed copy
 * constructor (MyDate) - sets date to same as object passed in toString ():
 ...
 * month and year as needed. isEqual (MyDate): boolean - returns true/false
 * whether parameter and object being acted on have equal values
 * isGreaterThan(MyDate): boolean - returns true/false whether object values are
 * greater than parameter object values *
 *
 ************************************************************************************************************
 */
public class MyDate {
	...
	public MyDate(int day, int month, int year) {
		if (year > 0) {
			...
			this.year = 2018; // parameter was invalid - so set to valid year
		}
		if (month > 0 && month < 13) {
			this.month = month;
		} else {
			this.month = 1; // parameter was invalid - so set to valid  month
		}
		if (day > 0 && day <= 31) {
			this.day = day;
		} else {
